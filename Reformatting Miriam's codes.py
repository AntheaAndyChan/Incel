# -*- coding: utf-8 -*-
"""
Created on Tue Sep 22 23:38:54 2020
Process Miriam's Codes'
@author: anthe
"""
...
'''Farrell & Miriam's Incel categories'''
jargon_codes = pd.read_csv(r'Miriam_Jargon_categorised.csv')
jargon_codes["Codes"]=jargon_codes["Codes"].str.lower()
jargon_codes["Codes"]=jargon_codes["Codes"].str.strip()
#pd.crosstab(index=miriam_codes["Codes"], columns="count") #categories of codes

jargon_codes["coh"]=np.where(jargon_codes['Codes']== 'coh', 1, 0)
jargon_codes["com"]=np.where(jargon_codes['Codes']== 'com', 1, 0)
jargon_codes["dep"]=np.where(jargon_codes['Codes']== 'dep', 1, 0)
jargon_codes["det"]=np.where(jargon_codes['Codes']== 'det', 1, 0)
jargon_codes["dow"]=np.where(jargon_codes['Codes']== 'dow', 1, 0)
jargon_codes["fr"]=np.where(jargon_codes['Codes']== 'fr', 1, 0)
jargon_codes["loa"]=np.where(jargon_codes['Codes']== 'loa', 1, 0)
jargon_codes["loa - class"]=np.where(jargon_codes['Codes']== 'loa - class', 1, 0)
jargon_codes["loa - mh"]=np.where(jargon_codes['Codes']== 'loa - mh', 1, 0)
jargon_codes["loa - pa"]=np.where(jargon_codes['Codes']== 'loa - pa', 1, 0)
jargon_codes["loa - pol"]=np.where(jargon_codes['Codes']== 'loa - pol', 1, 0)
jargon_codes["loa - race"]=np.where(jargon_codes['Codes']== 'loa - race', 1, 0)
jargon_codes["loa - sm"]=np.where(jargon_codes['Codes']== 'loa - sm', 1, 0)
jargon_codes["member"]=np.where(jargon_codes['Codes']== 'member', 1, 0)
jargon_codes["mh"]=np.where(jargon_codes['Codes']== 'mh', 1, 0)
jargon_codes["of"]=np.where(jargon_codes['Codes']== 'of', 1, 0)
jargon_codes["tm"]=np.where(jargon_codes['Codes']== 'tm', 1, 0)
jargon_codes=jargon_codes.drop(columns=["Codes"])
#print(miriam_codes.head())

jargon_codes.columns=['word','Hegemony_Conflict', 'Masculinity_Conflict','Depravity',
                     'Determinism','Dehumanize Women', 'Dissatisfaction w/ Fam',
                     'Law of Attraction(LOA)','LOA-Class','loa - mh','LOA-Physique',
                     'loa - pol','LOA - Race','LOA-Other',
                     'Members','Mental Health','Anti-feminism','Trolling']
jargon_codes['word'] = jargon_codes['word'].replace(['mghow'],'mgtow') #fix misspelling?
jargon_codes['word'] = jargon_codes['word'].replace(['tryone'],'tyrone') #fix misspelling?

'''Recode small categories and delete'''
jargon_codes.loc[jargon_codes.word == "anxietycel", "loa - mh"] = 0 #anxietycel 
jargon_codes.loc[jargon_codes.word == "anxietycel", 'Mental Health'] = 1 
jargon_codes.loc[jargon_codes.word == "blackopscel", "loa - pol"] = 0 #move blackopscel to other or physique
jargon_codes.loc[jargon_codes.word == "blackopscel", 'LOA-Physique'] = 1
jargon_codes.loc[jargon_codes.word == "nazicel", "loa - pol"] = 0 #anxietycel 
jargon_codes.loc[jargon_codes.word == "nazicel", 'LOA - Race'] = 1 
jargon_codes.loc[jargon_codes.word == "nazicels", "loa - pol"] = 0 #move blackopscel to other or physique
jargon_codes.loc[jargon_codes.word == "nazicels", 'LOA - Race'] = 1
jargon_codes=jargon_codes.drop(['loa - mh'], axis=1)
jargon_codes=jargon_codes.drop(['loa - pol'], axis=1)
#altrightcel- is that really LOA? where do nazicels go? stormfrontcel?
#blackopscel feels more like physique to me, because he was known among the uglycels
jargon_codes.to_csv('Jargon_codes.csv')
...
#miso_titles=['Belitting','Flipping the narrative','Homophobia','Hostility',
#            'Patriarchy','Physical Violence','Racism','Sexual Violence','Stoicism']
# totalwords=0
# for key, value in CATEGORY_WORDS.items():
#     totalwords+=len(value)
# print("Total words: ",totalwords) #1300

def process_jargon(miso_dict):
    miso_codes=pd.DataFrame()
    for key, value in miso_dict.items():
        df = pd.DataFrame(columns=["word", key])   
        for word in value:
            tmpword=pd.DataFrame([(word,1)], columns=['word', key])
            df=df.append(tmpword, ignore_index = True)
        miso_codes=miso_codes.append(df, ignore_index = True)
    miso_codes=miso_codes.fillna(0)
    return miso_codes
Misogyny_codes = process_jargon(CATEGORY_WORDS)
Misogyny_codes.to_csv('Misogyny_codes.csv')
